...
public class AirplaneAdapter extends SelectableAdapter<RecyclerView.ViewHolder> {
    ...
    public AirplaneAdapter(Context context, List<AbstractItem> items, OnSeatSelected onSeatSelected) {
        this.mOnSeatSelected = onSeatSelected;
        mContext = context;
                ...
                public void onClick(View v) {
                    try {
                        if (getSelectedItemCount() < 6) {
                            ...
                            mOnSeatSelected.onSeatSelected(getSelectedItemCount(), label, isSelected(position));
                        } else {
                            Toast.makeText(mContext, "Not more than 6", Toast.LENGTH_SHORT).show();
                        }
            ...
            holder.imgSeat.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    try {
                        if (getSelectedItemCount() < 6) {
                            toggleSelection(position);
                            mOnSeatSelected.onSeatSelected(getSelectedItemCount(), "", isSelected(position));
                        } else {
                            Toast.makeText(mContext, "Not more than 6", Toast.LENGTH_SHORT).show();
                        }
                    } catch (Exception e) {
    ...
    public interface OnSeatSelected {
        ...
        void onSeatSelected(int count, String lbl, Boolean selection);
    }
